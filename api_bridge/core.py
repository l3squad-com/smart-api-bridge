...
class APIBridge:
    ...
    def _setup_routes(self):
        self.router.add_api_route(
            f"{self.base_endpoint}/test", self.test_db_connection, methods=["GET"]
        )
        self.router.add_api_route(
            f"{self.base_endpoint}/{{table_name}}",
            self.get_all_records,
            methods=["GET"],
        )
        self.router.add_api_route(
            f"{self.base_endpoint}/{{table_name}}", self.create_record, methods=["POST"]
        )
        self.router.add_api_route(
            f"{self.base_endpoint}/{{table_name}}/{{record_id}}",
            self.update_record,
            methods=["PUT"],
        )
        self.router.add_api_route(
            f"{self.base_endpoint}/{{table_name}}/{{record_id}}",
            self.soft_delete_record,
            methods=["DELETE"],
        )
        self.router.add_api_route(
            f"{self.base_endpoint}/{{table_name}}/{{record_id}}/hard",
            self.delete_record,
            methods=["DELETE"],
        )
        self.router.add_api_route(
    ...
    def get_all_records(
        self, table_name: str, page: int = Query(1, ge=1), limit: int = Query(10, ge=1)
    ):
        ...
        try:
            ...
            pagination = {
                ...
                + (1 if total_records % limit else 0),
                "current_page": page,
            }
            ...
            raise HTTPException(
                status_code=500, detail=f"Error reading table: {str(e)}"
            )
        finally:
            session.close()
    ...
    def create_record(self, table_name: str, record: Dict[str, Any] = Body(...)):
        ...
        try:
            ...
            session.rollback()
            raise HTTPException(
                status_code=500, detail=f"Error inserting record: {str(e)}"
            )
        finally:
            session.close()

    def update_record(
        self, table_name: str, record_id: int, record: Dict[str, Any] = Body(...)
    ):
        session = self.Session()
        try:
    ...
    def soft_delete_record(
        self, table_name: str, record_id: int, payload: BaseModel = Body(...)
    ):
        session = self.Session()
        try:
            ...
            session.rollback()
            raise HTTPException(
                status_code=500, detail=f"Error soft deleting record: {str(e)}"
            )
        finally:
            session.close()
    ...
    def patch_record(
        ...
        try:
            ...
            session.rollback()
            raise HTTPException(
                status_code=500, detail=f"Error patching record: {str(e)}"
            )
        finally:
            session.close()
